...
class Subscriber(Observable):
	...
	def parse_payload(message) -> dict:
		...
		assert set(payload.keys()) == {'timestamp', 'sensors', 'actuators', 'controllers'}
		assert isinstance(payload['timestamp'], int) or isinstance(payload['timestamp'], float)
		assert isinstance(payload['sensors'], dict)
		assert isinstance(payload['actuators'], dict)
		assert isinstance(payload['controllers'], dict)

		parsed = {'timestamp': int(payload['timestamp']),
